...
import { ArrowDown, Terminal, Sparkles, Box, ShieldCheck, ChevronRight, Video, Flame } from "lucide-react";
    ...
    <section id="hero" className="relative min-h-screen pt-16 sm:pt-20 flex flex-col justify-between overflow-hidden bg-tech-grid">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 sm:pt-12 w-full">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4 sm:mb-6 font-mono text-xs">
          <div className="flex items-center gap-2 px-3 py-1 bg-black/60 border border-white/15 text-slate-300">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping" />
            <span className="text-amber-400 font-bold font-dot tracking-wider">{PROFILE.name}</span>
            <span className="text-slate-600">//</span>
            <span className="text-slate-300 font-mono tracking-tight">{PROFILE.degree.toUpperCase()}</span>
          </div>
        ...
        {/* Large Editorial Dev Title */}
        <div className="space-y-1 sm:space-y-2">
          <h1 className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-black tracking-tighter text-white uppercase leading-[0.92] font-tech">
            I BUILD WORLDS
        ...
        <div className="mt-4 sm:mt-6 flex items-center gap-3 font-mono text-xs sm:text-sm">
          <span className="text-emerald-400 font-bold tracking-widest shrink-0 font-dot">
            CURRENTLY BUILDING →
          </span>
          <div className="h-6 overflow-hidden relative w-full max-w-md">
            <div 
              key={statusIdx}
              className="text-white font-medium inline-block bg-black/70 px-2.5 py-0.5 border border-white/15 text-xs font-mono"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-end border-t border-white/10 pt-6">
          ...
          <div className="md:col-span-6 space-y-2">
            <div className="text-[11px] font-mono tracking-widest text-slate-500 uppercase">
              THE MANIFESTO // DUALITY PERSPECTIVE
            </div>
          ...
          <div className="md:col-span-6 flex flex-wrap items-center justify-start md:justify-end gap-3 font-mono text-xs">
            <button
              onClick={() => scrollToSection("environment-art")}
            ...
            <button
              ...
              className="p-2.5 border border-white/10 hover:border-white/30 text-slate-400 hover:text-white transition-colors cursor-pointer"
